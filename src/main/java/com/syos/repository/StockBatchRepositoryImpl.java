...
public class StockBatchRepositoryImpl implements StockBatchRepository {
	...
	public List<StockBatch> findByProduct(String code) {
		String sql = "SELECT id, product_code, purchase_date, expiry_date, quantity_remaining FROM stock_batches WHERE product_code = ? AND quantity_remaining > 0";
	...
	public List<StockBatch> findByProductAllBatches(String code) {
		String sql = "SELECT id, product_code, purchase_date, expiry_date, quantity_remaining FROM stock_batches WHERE product_code = ?";
		List<StockBatch> out = new ArrayList<>();
		try (Connection connection = DatabaseManager.getInstance().getConnection();
				PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
	...
	public void createBatch(String productCode, LocalDate purchaseDate, LocalDate expiryDate, int quantity) {
		String sql = "INSERT INTO stock_batches (product_code, purchase_date, expiry_date, quantity_received, quantity_remaining) VALUES (?,?,?,?,?)";
		try (Connection connection = DatabaseManager.getInstance().getConnection();
				PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
	...
	public List<StockBatch> findExpiringBatches(String productCode, int daysThreshold) {
		String sql = "SELECT id, product_code, purchase_date, expiry_date, quantity_remaining FROM stock_batches WHERE product_code = ? AND quantity_remaining > 0 AND expiry_date <= ? ORDER BY expiry_date ASC";
		List<StockBatch> out = new ArrayList<>();
		try (Connection connection = DatabaseManager.getInstance().getConnection();
				PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
	...
	public List<StockBatch> findAllExpiringBatches(int daysThreshold) {
		String sql = "SELECT id, product_code, purchase_date, expiry_date, quantity_remaining FROM stock_batches WHERE quantity_remaining > 0 AND expiry_date <= ? ORDER BY expiry_date ASC, product_code ASC";
		List<StockBatch> out = new ArrayList<>();
		try (Connection connection = DatabaseManager.getInstance().getConnection();
				PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
	...
	public StockBatch findById(int batchId) {
		String sql = "SELECT id, product_code, purchase_date, expiry_date, quantity_remaining FROM stock_batches WHERE id = ?";
		try (Connection connection = DatabaseManager.getInstance().getConnection();
				PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
