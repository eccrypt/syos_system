...
class BillingRepositoryTest {
	...
	void setUp() throws SQLException {
		...
		doNothing().when(mockPsBill).close();
		doNothing().when(mockPsItem).close();
	...
	void save_billSaveFails_rollbacksTransaction() throws SQLException {
		...
		assertTrue(thrown.getMessage().contains("Error saving bill & items"),
				"Exception message should indicate save error");
	...
	void nextSerial_existingBills_returnsMaxPlusOne() throws SQLException {
		...
		when(mockConnection.prepareStatement(
				contains("SELECT COALESCE(MAX(serial_number), 0) + 1 FROM bill WHERE DATE(bill_date) = CURRENT_DATE")))
	...
	void nextSerial_queryFails_throwsRuntimeException() throws SQLException {
		...
		assertTrue(thrown.getMessage().contains("Error generating daily serial"),
				"Exception message should indicate serial error");
		assertTrue(thrown.getCause() instanceof SQLException, "Cause should be SQLException");
		verify(mockConnection).close();
		verify(mockPsBill).close();
		verify(mockRs, never()).close();
